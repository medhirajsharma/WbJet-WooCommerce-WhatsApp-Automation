...
$notification = null;
if (isset($_GET['id'])) {
    $notification_id = absint($_GET['id']);
    if ($notification_id > 0) {
        $notification = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_name WHERE id = %d", $notification_id));
        if ($notification) {
            $notification->variable_mappings = !empty($notification->variable_mappings) ? json_decode($notification->variable_mappings, true) : array();
        }
    }
}
...
<div class="wrap WBWWA-admin">
            ...
            <div class="trigger-form-premium-card">
                <div class="trigger-form-hero">
                    ...
                    <div class="hero-content">
                        ...
                        <p class="hero-subtitle">Send WhatsApp notifications to your business when an order status changes or for special events.</p>
                    </div>
                </div>
                <form method="post" action="" autocomplete="off">
                                ...
                                foreach ($templates as $template) {
                                    ...
                                    if ($metadata) {
                                        printf(
                                            ...
                                            selected($notification ? $notification->message_template : '', $template['uuid'] ?? '', false),
                    ...
                    <div class="form-section-premium">
                        ...
                        <div class="form-switch-premium">
                            <label class="switch">
                                <input type="checkbox" name="is_active" value="1" <?php checked($notification ? $notification->is_active : true); ?>>
    ...
    jQuery(document).ready(function($) {
        ...
        const savedMappings = <?php echo $notification && is_array($notification->variable_mappings) ? wp_json_encode($notification->variable_mappings) : 'null'; ?>;
