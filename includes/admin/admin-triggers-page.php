<?php
// This file renders the Triggers admin page for SwiftChats
// (Copy the triggers page code from admin-menu.php here)
...
$total_triggers = $wpdb->get_var("SELECT COUNT(*) FROM $table_name");
$total_pages = ceil($total_triggers / $triggers_per_page);
$offset = ($paged - 1) * $triggers_per_page;
$triggers = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name ORDER BY id DESC LIMIT %d OFFSET %d", $triggers_per_page, $offset));
settings_errors('swiftchatswc_messages');
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete' && !empty($_POST['trigger_id'])) {
    if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'swiftchats_trigger_nonce')) {
        ...
        $deleted = $wpdb->delete($table_name, array('id' => $trigger_id));
        if ($deleted) {
            wp_redirect(admin_url('admin.php?page=swiftchatswc-triggers&msg=deleted'));
            exit;
        } else {
            add_settings_error('swiftchatswc_messages', 'db', 'Failed to delete trigger.', 'error');
        }
    }
}
?>
    ...
    <div class="swiftchats-hero-card">
        ...
        <div class="hero-content">
            ...
            <p class="hero-subtitle">
                WhatsApp notifications sent to your customers when order status changes or for special events (e.g. Abandoned Cart).
            </p>
        </div>
        <?php if (!$has_api_error): ?>
            <a href="<?php echo esc_url(admin_url('admin.php?page=swiftchatswc-trigger-edit')); ?>" class="button swiftchats-modern-btn add-notification-btn">
                <span class="dashicons dashicons-format-chat"></span> <span>Add New Trigger</span>
            </a>
        <?php endif; ?>
    </div>
    <?php if ($has_api_error): ?>
        <div class="swiftchats-card swiftchats-api-notice">
            ...
            <p>To start creating triggers, you need to configure the API settings first.</p>
                    ...
                    <?php if (empty($triggers)) : ?>
                        <tr>
                            <td colspan="4" class="empty-table">
                                ...
                                No triggers found. <a href="<?php echo esc_url(admin_url('admin.php?page=swiftchatswc-trigger-edit')); ?>">Add one?</a>
                            </td>
                        </tr>
                    <?php else : ?>
                        <?php foreach ($triggers as $trigger) : ?>
                            <tr>
                                <td>
                                    <?php
                                    $status_key = $trigger->order_status;
                                ...
                                <td><?php echo esc_html($trigger->message_template_name ?? ''); ?></td>
                                <td>
                                ...
                                <td>
                                    <a href="<?php echo esc_url(admin_url('admin.php?page=swiftchatswc-trigger-edit&id=' . $trigger->id)); ?>"
